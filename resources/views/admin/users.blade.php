                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex gap-2 justify-center">
                                    ...
                                    <form method="POST" action="{{ route('admin.users.reject', $user->id) }}" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700" onclick="return confirm('Apakah Anda yakin ingin me-reject user ini?')">
                                            Reject
                                        </button>
                                    </form>
                                </div>
                    ...
                    <div class="flex justify-between items-center border rounded-lg p-3">
                        ...
                        <div class="flex gap-2 items-center">
                            ...
                            <form method="POST" action="{{ route('admin.users.destroy', $user->id) }}" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button class="text-xs px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700" onclick="return confirm('Apakah Anda yakin ingin menghapus user ini?')">
                                    Delete
                                </button>
                            </form>
                        </div>
